...
struct ThreadPoolReal {
  ...
  ThreadPoolReal(int64_t queue_size) :
...
static void thread_main(void* data) {
  ThreadPoolReal* pool = (ThreadPoolReal*)data;
...
ThreadPool* create_thread_pool(int thread_count, int queue_size) {
  ThreadPoolReal* thread_pool = new ThreadPoolReal(queue_size);
...
void destroy_thread_pool(ThreadPool* thread_pool_ptr) {
  ThreadPoolReal* thread_pool = (ThreadPoolReal*)thread_pool_ptr;
...
int thread_pool_thread_count(ThreadPool* thread_pool_ptr) {
  return ((ThreadPoolReal*)thread_pool_ptr)->thread_count;
}
